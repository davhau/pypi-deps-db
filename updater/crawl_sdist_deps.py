...
def compute_drvs(jobs: List[PackageJob], extractor_src, store=None):
    ...
    with TemporaryDirectory() as tempdir:
        ...
        cmd = [
            ...
            "--json",
            "--expr",
            f"import {extractor_src}/make-drvs.nix {{ pypiData = ./.; }}",
        ]
        if store:
            cmd += ["--store", store]
...
def format_error(log: str, pkg_version):
    ...
    common = ("unpacker produced multiple directories",)
    for err in common:
        if err in log:
            log = err
            break
    ...
    filtered = filter(
        lambda l: not any(marker in l for marker in remove_lines_marker), lines
    )
...
def extract_requirements(job: PackageJob, deadline, total_num, store, extractor_src):
    try:
        ...
        print(
            f"Bucket {job.bucket} - Job {job.idx+1}/{total_num} - "
        ...
        with TemporaryDirectory() as tempdir:
            ...
            cmd = [
                ...
                "-L",
                "-f",
                f"{extractor_src}/fast-extractor.nix",
                "--arg",
                "pypiData",
                "./.",
                "--argstr",
            ...
            try:
                ...
                if any(
                    s in formatted
                    for s in ("o space left on device", "lack of free disk space")
                ):
                    return e
                return [
                    JobResult(
...
def get_jobs(
    pypi_index, error_dict, pkgs_dict, bucket, py_vers, limit_num=None, limit_names=None
):
    ...
    print(
        f"Bucket {bucket}: {len(jobs)} out of {total_nr} total sdist releases need to be updated"
    )
    return jobs
...
def build_base(extractor_src, py_vers, store=None):
    ...
    version = "2.22.0"
    url = (
        "https://files.pythonhosted.org/packages/01/62/"
    ...
    cmd = [
        ...
        "--arg",
        "pyVersions",
        f"""[ {" ".join(map(lambda p: f'"{p}"', py_vers))} ]""",
        "--no-out-link",
    ]
    if store:
        cmd += ["--store", f"{store}"]
...
def main():
    ...
    limit_names = set(
        filter(lambda n: bool(n), os.environ.get("LIMIT_NAMES", "").split(","))
    )
    ...
    py_vers_short = (
        os.environ.get("PYTHON_VERSIONS", "27,36,37,38,39,310").strip().split(",")
    )
    ...
    for idx, bucket in enumerate(LazyBucketDict.bucket_keys()):
        ...
        pypi_index = LazyBucketDict(
            f"{pypi_fetcher_dir}/pypi", restrict_to_bucket=bucket
        )
        # load error data
        ...
        with Measure("executing jobs"):
            if workers > 1:
                ...
                pool_results = [
                    extract_requirements(
                        args, deadline, len(jobs), store, extractor_src
                    )
                    for args in jobs
                ]
        ...
        # insert new data
        for pkg in sorted(
            results, key=lambda pkg: (pkg.name, pkg.version, sort_key_pyver(pkg.py_ver))
        ):
            ...
            if pkg.error:
                ...
                target = pkgs_dict
            insert(
                py_ver, pkg.name, pkg.version, pkg_to_dict(pkg), target, error=pkg.error
            )
